...
from src.data_processor.llama_ultra_chat import UltrachatTrain
from src.data_processor.llama_alpaca import AlpacaTrain, AlpacaEval
from src.data_processor.collator import LongestSequenceCollator, MoeCollator
import torch
...
def main(parser):
    ...
    print(device_map)
    if model_args.is_debug:
        model = LlamaForCausalLM(config)

    else:
        model = LlamaForCausalLM.from_pretrained("/opt/data/private/wcy/models/Llama-2-7b-hf",
    ...
    #                                              response_column, history_column, prefix,
    #                                              tokenizer, task_flag, model_args.department)
    # elif "llama" in model_args.model_name_or_path.split("/")[-1].lower():
    #     preprocess_function_train = llama_train(data_args, model_args, prompt_column,
    #                                                response_column, history_column, prefix,
    #                                                tokenizer, task_flag)
    #     preprocess_function_eval = llama_eval(data_args, model_args, prompt_column,
    #                                              response_column, history_column, prefix,
    #                                              tokenizer, task_flag)
    # else:
    #     raise ValueError("No such Foundation Model")

    if model_args.data_type == "alpaca":
        preprocess_function_train = AlpacaTrain(data_args, model_args, prompt_column,
                                            response_column, history_column, prefix,
                                            tokenizer, task_flag)
        preprocess_function_eval = AlpacaEval(data_args, model_args, prompt_column,
                                                response_column, history_column, prefix,
                                                tokenizer, task_flag)
    elif model_args.data_type == "ultrachat":
        preprocess_function_train = UltrachatTrain(data_args, model_args, prompt_column,
                                            response_column, history_column, prefix,
                                            tokenizer, task_flag)
    ...
    if training_args.do_train:
        ...
        with training_args.main_process_first(desc="train dataset map pre-processing"):
            train_dataset = train_dataset.map(
                ...
                desc=f"Running tokenizer on train dataset on {device_map}",
            )
